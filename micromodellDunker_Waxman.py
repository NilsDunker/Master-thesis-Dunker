...
save_pic_dir = project_path + '/Plots/'
...
class Micromodel:
    ...
    def __init__(self, number_of_nodes, average_degree,
                 ...
                 timesteps, inactivity_time, gif_duration, waxman_alpha, waxman_beta, waxman_L, shock_probability,
    ...
    def _one_run(self,
        ...
        for timesteps in range(0, self._timesteps):
            ...
            self.create_shock(inactive_nodes, newly_active_nodes)
    ...
    def create_shock(self, inactive_nodes, newly_active_nodes):
        shocknodes = np.random.choice(list(inactive_nodes), 3)
        for shocknode in shocknodes:
            if np.random.rand() < self._shock_probability:
                newly_active_nodes.add(shocknode)
    ...
    def create_frame(self, certainly_active_nodes, certainly_inactive_nodes, network, pos, inactive_nodes, active_nodes,
        ...
        plt.tight_layout()
        plt.savefig(save_pic_dir + 'Waxman_GIF/jpgs/timestep{0}_{1}.jpg'.format(timesteps, certainly_active))
        plt.clf()
        plt.show()
    ...
    def create_gif(self, c):  # komprimiert die Frames zu einem GIF
        ...
        save_dir = save_pic_dir + 'Waxman_GIF/'
...
if __name__ == "__main__":  # Python files can act as either reusable modules, or as standalone programs-> ich könnte also "micromodel" als Modul importieren um von ihm gebracuh zu machen-->http://effbot.org/pyfaq/tutor-what-is-if-name-main-for.htm
    ...
    m = Micromodel(number_of_nodes=2000, average_degree=5,
                   ...
                   waxman_alpha=0.1, waxman_L=None, waxman_beta=0.45,
...
# todo: Ist der average Degree überhaupt richtig eingebaut? -> richtig einbauen
